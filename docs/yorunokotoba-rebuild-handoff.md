...
| 廃止（機能削除）| お守り / 今夜のいい時間帯 |

キャラクターは残す（オーナー明示）。相性占いは「無料機能として残す」（オーナー指示。過去の削除指示は CompatibilityCard = シェアカードのみで、相性占い機能自体は残す）。機能の Premium / 無料設計は §3-1 を参照。
...
| 星座占い | 今日の運勢を読む | ランク + キャラ + 恋愛/仕事/健康 + ホロスコープ（星座の本質）| お守り |
| 相性占い（無料）| 自分と相手の星座で相性を見る | 星座の相性スコア + エレメント構成 + ペアのコメント。Tinder の占星術 UI を参考 | 課金壁（相性占いは全て無料）|
| 履歴 | 過去の夢/占いを見返す | 日付順リスト、夢の内容が主役 | 過剰な装飾 |
| 設定 | 解約・規約確認 | アカウント・Premium状態・法務 | ナイトモードトグル（廃止）|
| Premium | 課金する | 月¥980の価値 = 夢記録 + ホロスコープ深層心理 + キャラからの今月メッセージ、CTA1つ | 機能羅列 |

画面遷移は `App.tsx` の `useState<ViewKey>` を維持しつつ、結果系画面を `history.pushState` で戻る対象にする（iOS スワイプバック対応）。

## 3-1. 機能の Premium / 無料設計（オーナー確定）

| 区分 | 機能 |
|---|---|
| 無料 | 夢占い（基本のよみとき）/ 星座占い（今日の運勢 + ホロスコープの基本）/ 相性占い（星座の相性スコア + エレメント構成。Tinder の占星術を参考）/ 履歴の基本閲覧 |
| Premium（月¥980）| ① 夢記録: 夢日記を「傾向が振り返れる」記録機能に（蓄積した夢の傾向分析）② ホロスコープのより深層心理 ③ キャラクター（守護キャラ、guardianMessages のキャラ）からの今月メッセージ |

相性占いは全て無料（課金壁なし）。Premium の壁は「夢記録の傾向分析 / ホロスコープ深層心理 / キャラの今月メッセージ」の3点。Tinder のように相性の一部をロックする方式は採らない。

相性占いの内容（Tinder 占星術 参考）: 太陽星座での性格の相性 → 星座の相性スコア（Vibes / コミュニケーション 等を5段階バー）→ エレメント構成（火/地/風/水 の比率）→ ペアへの一言コメント。出生時刻/場所での月星座・アセンダント拡張は将来検討（初期は太陽星座のみで無料完結）。
