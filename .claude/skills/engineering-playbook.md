...
> **アイデンティティ原則:** tech-lead / fullstack-dev / ai-engineer / infra-devopsはそれぞれ固有の
> キャラクター設定（経歴・核心的信念・思考習慣）を持つ。このPlaybookは技術判断の共通基盤として機能し、
> 各エージェントのアイデンティティと組み合わせることで一貫したエンジニアリング文化を形成する。
...
## 3.5 実装前チェック義務（反証モード・コード版）

> **コードを1行でも書く前に以下を実行すること。省略禁止。**
> これがフリーズ・バグ・クラッシュを防ぐ最重要プロセス。

### 実装前に必ず答える3つの質問

```
1. この実装が壊れる最も可能性の高いケースは何か？
   → null/undefined、空配列、ネットワーク失敗、未認証状態、同時実行

2. エッジケースで何が起きるか？
   → 0件/大量件数のデータ、タイムアウト、部分的な失敗、リトライ時の重複

3. 本番環境で問題になることは何か？
   → パフォーマンス劣化、メモリリーク、競合状態、副作用
```

### フロントエンド実装時の追加義務

`frontend-quality-guard.md` を参照し、以下を確認してからコードを書く:

```
- [ ] useEffectの依存配列は安全か？（オブジェクト/関数を直接入れない）
- [ ] 非同期処理のクリーンアップはあるか？
- [ ] loading/error/successの3状態を全て処理するか？
- [ ] ErrorBoundaryで包まれる位置にあるか？
- [ ] React Query / AbortController を使っているか？
```

### 必須パッケージ（フロントエンド・毎回確認）
...
この3つが入っていないプロジェクトへのReact実装は、まずインストールから始める。
