...
### 3.5 外部ツール採用チェックリスト（2026-05-18 追加、lazyweb / Canva 失敗の構造対策）

外部ツール（MCP / plugin / 外部サービス）を ConsultingOS 規律で第一選択 / 必須 / 標準として物理化する前に、必ず以下を実測する（CLAUDE.md ハードルール 5）。

- [ ] ① 生成・呼出: その実行環境でツールを実際に呼ぶ（例: `generate-design` を実行）
- [ ] ② 成果物取り込み: 生成物を ConsultingOS の実行コンテナに取り込むコマンドを実行（例: 出力 URL を WebFetch / ダウンロード）
- [ ] ③ 実出力検証: 取り込んだ成果物を grep / ファイル確認で検証（403 等の取り込み失敗を検査項目に含める）
- [ ] 実測コマンドと出力を反証 Step 3 に添付（「help を見た」「公式が言っている」は実測ではない）

NG 事例: lazyweb MCP（ローカルプラグイン非永続を未検証で物理化、PR #237 不採用）/ Canva MCP（help 確認のみで第一選択化、generate-design 出力が design.canva.ai 403 で取り込み不可、PR #245 撤回）。「マネージド型なら安全」も誤り: Canva はマネージド型でも取り込み 403。判定軸は型ではなく「取得経路まで end-to-end で実測したか」。
