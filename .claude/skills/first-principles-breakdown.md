...
## §ASI時代への適用：「前提を剥がす」最重要フロンティア

> AGIが「与えられた問題を解く」能力でほぼ全ての人間を超えた時代、人間の最後の競争優位は「問いを設定する能力」になる。第一原理分解は、この「問いの設定」のための最重要ツールになる。

### ASI時代に特に剥がすべき前提

```
AGI普及後、最も危険な前提は「AI以前の常識」がそのまま通用するという思い込み。

【ビジネスモデルの前提】
「収益はサービスを売ることで得る」
→ AGI後: 知識・労働力のコストがゼロに近づく。「誰が価値を占有できるか」から問い直す

【人材の前提】
「優秀な人材 = 高度なスキルを持つ人」
→ AGI後: スキルはAIがカバー。「問いを立て・意思決定し・責任を取れる人」が希少

【競争の前提】
「競合との差別化 = 機能・品質・価格」
→ AGI後: 機能・品質・価格はAIがコモディティ化。「信頼・文化・関係性」が差別化源

【市場の前提】
「大きな市場 = チャンスがある」
→ AGI後: AIが大市場を最速で侵食。「AIが入れない隙間」か「AIより信頼される理由」を探す
```

### ASI時代の第一原理分解テンプレート

```
Step 0（ASI時代追加）: AGI後世界での問いの再定義
  「この問いは、AIが代替した後でも問い続ける価値があるか？」
  「AIが最適解を出した後に、人間が判断すべきことは何か？」

Step 1〜5（従来通り実行）

Step 5.5（ASI時代追加）: Human Moat検証
  再構築した解決策は「AIが代替できない人間的価値」を含んでいるか？
  含んでいない場合: AGI後に競争優位がゼロになるリスクがある
  → Trust Premium / 物理的世界 / 文化的権威 / データループ / 規制の5軸で検証
```
...
```
AGI前の仕事: 「正しい答えを最速で出す」
AGI後の仕事: 「正しい問いを設定し、AGIに解かせ、その答えを批判的に評価する」

訓練すべき問い:
1. 「これは本当に解くべき問いか？」（問いの妥当性検証）
2. 「この問いを解くと、誰が何を得て、誰が何を失うか？」（ステークホルダー分析）
3. 「AGIが最適解を出したとして、なぜ私はそれを信じるべきか（疑うべきか）？」
4. 「AGIが答えを出せない問いはどれか？」（Human Moatの発見）
```
