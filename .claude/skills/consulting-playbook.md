...
- **プロダクトバリュー定期更新**: 2 年弱に 1 度プロダクト価値を柔軟に変化させる（FreakOut 7 年で 4 回実証）。技術・ユーザー環境・市場構造の変化に追従、「成功体験」に固執しない
- **先行事例マーケット構造比較**: 新市場分析はゼロから推測せず、類似先行事例のマーケット構造を比較分析（例: VR 市場を PlayStation / Wii の普及曲線で読む）。参入タイミングは普及率の定量閾値で判断（例: スマホ浸透率 10% で参入）（出典: [ONE CAREER](https://www.onecareer.jp/articles/1583) / [CareerHack](https://careerhack.en-japan.com/report/detail/887) / [Schoo](https://schoo.jp/minute/226)）
...
## AI 派遣会社モデル + 守り → 攻め転換診断 (2026-05-22): 参入優位の公式「業界知識 × AI」(INFERENCE) = AI を売る側の真の moat は AI 技術でなくフリーランス (技術のみ・毎回ゼロ学習) vs 経営者 (業界知識 20 年 × AI) の参入優位構造。道 1 守り (自社業務に AI 組込み生産性向上) / 道 2 攻め (業界知識を AI 標準化し他社へ外販 = AI 派遣会社化) 判定ロジックをクライアント診断のサービス設計に適用、ICP §9.1 Vertical AI 適合度の上位概念。ConsultingOS 自身が「業界知識 × AI の AI 派遣会社」プレイの実装 (主語は自社とクライアント両方)。米国 $ 数値は SPECULATION (米国の個人事業者事例の紹介投稿、一次統計未確認・要検証)。詳細: [`references/consulting-playbook-ai-staffing-model.md`](references/consulting-playbook-ai-staffing-model.md)
## AI BPO 2 参入機会モデル + 6 ポイント (2026-05-22): 道 2（攻め = AI 派遣会社化）の市場・デリバリー構造の具体化。JAFCO レポート（種市 / 上岡）起点、市場フレーミング = INFERENCE、事例事実 = FACT（レポート記載）、将来予測 = SPECULATION。ゲームチェンジ: 人海戦術 BPO が AI で構造変化、勝ち筋は単価でなく「数量 × 粗利率」。参入機会 (i) エンタープライズ向け（大手寡占、数千人キャパ + HITL ハイブリッド、M&A でショートカット、事例 Crescendo: PartnerHero 買収で 3,000-3,500 名・200 社以上・FACT レポート記載値）/ (ii) SMB 向け新規 BPO（AI で初めて外注が経済成立、デファクト化スピード勝負、事例 Tennr: 医療紹介状 FAX 一気通貫 + RaeLM 専用モデル + ROI 月次可視化 + ワークフロー不変設計）。ConsultingOS 主軸 = (ii) SMB（ICP §9.1 中以上の業界・Tennr 型再現モデル）、エンタープライズ (i) はケイパ補完フック アライアンスで部分関与（M&A 非採用）。6 ポイント分担: マーケ選定 / 特化標準化（交点）/ 営業 / ロックイン（交点）/ ファイナンス = コンサル部門、粗利率（HITL + 推論コスト）= AI・サービス開発部門。接続: AI 派遣会社モデル（道 2 上位概念）→ 本ファイル（市場具体化）。詳細: [`references/consulting-playbook-ai-bpo-market-entry.md`](references/consulting-playbook-ai-bpo-market-entry.md)
