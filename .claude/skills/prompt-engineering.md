...
owner: "ai-engineer"
changelog:
  - "3.2.1: 拒否率 5%→2% 改善（トーン調整）"
  - "3.2.0: 多言語対応追加"
test_cases:
  - input: "返金したい"
    expected_contains: ["承りました", "確認"]
    expected_not_contains: ["できません"]
```

### プロンプトA/Bテスト設計
```
仮説: 「XXXという表現を追加すると YYY指標が ZZ% 改善する」

テスト設計:
- コントロール: 現行プロンプト
- 変数: 変更箇所は1つのみ（多変数テストは解釈不能になる）
- サンプル: 最低100リクエスト/バリアント
- 評価: 自動（LLM-as-judge）+ 人間評価サンプリング（5%）

判定基準:
- 改善 ≥ 5% かつ統計的有意（p < 0.05） → 本番採用
- 改善 < 5% → 現行維持
- 悪化 → 即時ロールバック
```
...
| 手法 | コスト削減効果 | 注意点 |
|---|---|---|
| プロンプト圧縮 | 30〜50% | 品質低下リスクあり・必ずA/Bテスト |
| レスポンスキャッシング | 最大95% | TTL設定・キャッシュ汚染に注意 |
| モデル階層化（ルーティング） | 60〜80% | 複雑タスクの判別精度が鍵 |
| バッチ処理 | 50% | リアルタイム性が不要なケースのみ |
| ストリーミング | コスト変化なし | UX改善（体感速度向上）に有効 |
...
class LLMGuardrail:
    ...
    def post_call(self, response: str) -> str:
        # 1. 有害コンテンツフィルタリング
        # 2. ハルシネーション疑いのフラグ付け
        # 3. PII漏洩チェック
        # 4. 期待フォーマット検証（JSON schemaバリデーション等）
...
| 指標 | 定義 | 目標値 |
|---|---|---|
| **Faithfulness** | 回答が検索結果に忠実か | ≥ 0.90 |
| **Answer Relevancy** | 回答が質問に答えているか | ≥ 0.85 |
| **Context Precision** | 検索されたチャンクの精度 | ≥ 0.80 |
| **Context Recall** | 必要な情報が検索されているか | ≥ 0.80 |

### チャンク戦略の選択
```
テキスト種別に応じたチャンク戦略:

技術文書・マニュアル → 固定長（512〜1024トークン）+ オーバーラップ10〜20%
会話・Q&A形式      → セマンティックチャンク（意味境界で分割）
長文レポート        → 階層チャンク（セクション要約 + 詳細の2層構造）
コード             → 関数・クラス単位（構文解析ベース）
```

### RAG品質チェックリスト
- [ ] チャンクサイズとオーバーラップを評価データで最適化したか
- [ ] 埋め込みモデルはドメイン特化型を検討したか
- [ ] ハイブリッド検索（ベクトル + BM25）を試したか
- [ ] リランキング（cross-encoder）でTop-K精度を改善したか
- [ ] ハルシネーション率をFaithfulnessスコアで定期計測しているか
