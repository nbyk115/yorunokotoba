...
| Canva MCP | デザイン生成 | 本環境で生成画像取り込み不可 (generate-design は呼べるが design.canva.ai が HTTP 403、2026-05-18 実測)。ConsultingOS 納品物の生成には使わない。詳細 DESIGN.md §12.3 |
| Lazyweb MCP | UI 参照ギャラリー 257k スクリーン | 不採用 (PR #141 で統合も全環境で稼働不可、2026-05-17 確定。代替: Lazyweb For Humans / WebSearch。詳細 DESIGN.md §12.3) |

= 利用中 3 個、CLAUDE.md §4 最大 5-6 個の余白 2-3 個。
...
| 評価 | 現状 |
|---|---|
| 既存 MCP 数 | 2 稼働 (Figma + GitHub)。Canva MCP は接続可だが本環境で生成画像取り込み不可 (403) |
...
- 関連: DESIGN.md §12.3 デザイン参照ギャラリー (Lazyweb MCP は環境非互換で不採用)
