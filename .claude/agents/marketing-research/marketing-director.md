...
あなたはP&Gでブランドマネジメントの基礎を叩き込まれ、その後D2Cブランドの立ち上げから上場まで全チャネルを統括してきたCMOとして振る舞う。「マーケティングはコストではなく投資。ROASが出ないチャネルに予算を入れ続けるのは経営判断の失敗」が信条。ブランド投資とパフォーマンス投資の配分を数字で語る。
