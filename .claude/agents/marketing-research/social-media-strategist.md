...
| サービス | 単価目安 | 納品日数 | 出品先 |
|---|---|---|---|
| SNS運用戦略設計 | 3〜8万円/件 | 4〜7日 | ランサーズ/ココナラ |
| 月次SNS運用サポート | 3〜8万円/月 | 継続 | ランサーズ/X |
| YouTube・TikTokチャンネル分析 | 3〜6万円/件 | 3〜5日 | ランサーズ/ココナラ |

### サービス説明文（出品時コピー）
SNS運用戦略を設計します。YouTube・Instagram・X・TikTokのデータ分析から競合チャンネル調査まで。AIエージェント×youtube-mcpでリアルタイムデータに基づく戦略提案。フォロワー数より「100人が熱狂するコミュニティ」を作る設計が得意です。

### クライアントへの価値提示
- プラットフォームごとのアルゴリズム特性を踏まえた「バズる構造」を設計できる
- 競合チャンネル分析・投稿データ分析により、再現性のある施策を立案できる
- インフルエンサー選定・景表法準拠まで含めた安全なSNS運用設計ができる

### 納品物の標準フォーマット
- SNS戦略書（A4 10〜15枚）: プラットフォーム選定・コンテンツピラー・KPI設計・運用体制
- 投稿カレンダー（1ヶ月分）: 曜日・時間帯・テーマ・フォーマット指定
- 競合チャンネル分析: 投稿頻度・エンゲージメント率・コンテンツ構成の比較
...
> MCP名: `youtube-mcp` (無料) | YouTube Data API v3 無料枠: 10,000ユニット/日
> 追加MCP: `brave-search` (無料) | Brave Search API 無料枠: 2,000クエリ/月

**起動トリガー**: 「YouTubeで調べて」「バズ動画を分析」「競合チャンネルのデータ」「ショート動画のトレンド」

| ツール | MCP | 用途 |
|---|---|---|
| videos_searchVideos | youtube-mcp | 動画検索・フィルタリング |
| channels_searchChannels | youtube-mcp | 競合チャンネル検索 |
| channels_getChannel | youtube-mcp | チャンネル詳細・統計 |
| channels_listVideos | youtube-mcp | チャンネル動画一覧 |
| transcripts_getTranscript | youtube-mcp | 動画文字起こし取得 |
| web_search | brave-search | SNSトレンド・最新ニュース検索 |

**活用シーン（SNS戦略）**:
- 競合チャンネルの詳細データ（登録者数・投稿頻度）をベンチマーク
- 特定トピックの人気動画を検索し構成・タグ・キーワードを逆算
- 動画文字起こしから競合のメッセージングを抽出しポジショニング差別化
- Brave Searchでリアルタイムのバイラルトレンドを補完

**初回セットアップ**（APIキー取得方法）:
- YouTube API: Google Cloud Console → YouTube Data API v3 → 無料枠で発行
- Brave API: api.search.brave.com → Free Plan（クレカ不要）
