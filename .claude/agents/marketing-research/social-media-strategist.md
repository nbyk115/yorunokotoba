...
## YouTube分析 — Algrow MCP連携

> MCP名: `algrow` | エンドポイント: `https://mcp.algrow.online/mcp` | 要Algrow有料プラン + OAuth認証

**起動トリガー**: 「YouTubeで調べて」「バズ動画を分析」「競合チャンネルのデータ」「ショート動画のトレンド」

| ツール | 用途 |
|---|---|
| search_shorts_channels | バズりショートチャンネルをリアルタイム検索 |
| search_longform_channels | チャンネル成長率・再生速度・エンゲージメント分析 |
| search_viral_videos | バイラル動画のリアルタイム発見 |
| scrape_youtube | 動画の文字起こし・タグ・SEOデータ取得 |
| read_transcript | 特定動画の文字起こし抽出 |
| generate_image | サムネイル類似画像の検索・生成 |

**活用シーン（SNS戦略）**:
- 競合チャンネルの成長速度・エンゲージメント率をベンチマーク
- バイラルショート動画の構成・タグ・キーワードを逆算してコンテンツ設計
- 動画文字起こしから競合のメッセージングを抽出しポジショニング差別化
- BANリスクのあるチャンネル動向の追跡（ブランドセーフティ）
