...
## X (Twitter) アルゴリズム 2026 年最新版 (24,000 行更新後、2026-05-15 統合 PR #200)

> 出典: ユーザー提示テキスト 2026-05-15 (INFERENCE、一次出典 fetch 未実施、Elon Musk 公開発表系統)。前 PR #192 で削除した旧 2023 オープンソース版とは内容差分あり。

### スコア加減算 (投稿ごと)

ポジティブアクション (加点):
- リプライ / リツイート / 写真クリック / プロフィールクリック / 投稿シェア / DM シェア / リンクコピー / 投稿長時間滞在

ネガティブアクション (減点):
- 素早くスクロール / 「興味がない」クリック / ブロック / ミュート / 通報

### 同作者連投スコア減衰

フィードは作者ごとに 1 投稿のみ表示しようとする、その後の投稿はスコア減衰。日中の連投はフィード表示確率低下、スパム化禁止。

### 広告安全性連動ランキング

投稿が広告主にとって親しみやすくない場合、ランキング変動。突飛な陰謀論は広告掲載されない = フィードランキング低下。

### バンガー検出 + スロップスコア

タグ / カテゴリ / 品質 / スロップスコアでスキャン。スロップ投稿は水面下で死ぬ。AI 動画 (ドラゴン vs 妖精 vs 騎士等) は要警戒、量より質。

### IN NETWORK ブースト復活

フォロワーの投稿が優遇 (フォロー外スコア減衰の修正)、フォロワー向けコンテンツが効果的。

### ペナルティラベル 5 種

NSFW / 残虐描写 / 暴力 / クリックベイト喧嘩動画 / AI コンテンツ排除。スロップスコアはコード内で複数回言及、X は AI コンテンツ排除に本格対応中。

### 関根さん N&Y Craft + 水野さん v4 への適用

- 関根さん: BtoB 製造業の X 運用は IN NETWORK + 長時間滞在投稿に集中、AI 生成画像 / 動画は最小限
- 水野さん: 投資 / AI 業界の X 運用で長時間滞在誘発 (詳細スレッド / データ可視化) + 広告安全性スコア確保
