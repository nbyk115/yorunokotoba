...
> AGI後、コンテンツ生成・キーワード調査・技術的最適化はAIが自動化される。SEO専門家の価値は「AIに選ばれるための構造設計」と「Trust Signalの構築」に転換される。
...
【Human SEOが担い続ける機能】
✅ 「AIに選ばれるための信頼構造」の設計
✅ E-E-A-T（経験・専門性・権威性・信頼性）の証明設計
✅ AI検索エンジンへの情報提供アーキテクチャ設計
✅ コミュニティ・外部リンクの信頼ネットワーク構築
```

**ASI時代のSEO戦略転換**:
- AIO（AI Optimization）: ChatGPT/Perplexity/Geminiに引用されるための構造設計
- Schema.org完全実装: AIが機械的に読める構造化データを全コンテンツに適用
- Trust Signal: 第三者引用・外部権威からのリンクがAI時代に最も重要なシグナル
- Human-first Content: AIが量産できる情報ではなく「経験・独自調査・一次情報」を優先
...
| サービス | 単価目安 | 納品日数 | 出品先 |
|---|---|---|---|
| SEO改善レポート | 3〜8万円/件 | 5〜7日 | ランサーズ/ココナラ |
| テクニカルSEO診断 | 5〜12万円/件 | 7〜10日 | ランサーズ/ココナラ |
| キーワード戦略設計 | 3〜6万円/件 | 3〜5日 | ランサーズ/ココナラ/X |

### サービス説明文（出品時コピー）
サイトのSEO課題を診断し改善策を提案します。技術面・コンテンツ面・被リンク面を網羅的に分析。Core Web Vitals対策・構造化データ・国際SEOにも対応。「テクニカルSEOを直してからコンテンツ投資」という正しい優先順位で、費用対効果の高い施策を設計します。

### クライアントへの価値提示
- クロール・インデックスの技術的問題を特定し、コンテンツ投資が無駄にならない土台を作れる
- Core Web Vitals改善により、ユーザー体験と検索順位を同時に向上できる
- AI検索（SGE/AIO）時代に対応した構造化データ・E-E-A-T強化まで一括で設計できる

### 納品物の標準フォーマット
- SEO診断レポート（A4 12〜18枚）: テクニカル健全性スコア・問題点一覧・改善ロードマップ
- 優先改善リスト: インパクト×実装コストで重み付けした施策一覧（即対応/短期/中期）
- キーワードマップ: トピッククラスター設計・検索意図分類・ギャップ分析
