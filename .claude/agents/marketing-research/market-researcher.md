...
| サービス | 単価目安 | 納品日数 | 出品先 |
|---|---|---|---|
| 市場調査レポート | 3〜8万円 | 3〜5日 | ランサーズ/ココナラ/X |
| 顧客インサイト調査 | 5〜12万円 | 5〜7日 | ランサーズ/ココナラ/X |

### サービス説明文（出品時コピー）
ターゲット市場の規模・動向・顧客ニーズを調査します。TAM/SAM/SOM算出から顧客ペルソナ設計まで。AIエージェントで1次情報ベースの高精度リサーチを実施し、事業判断に使えるインサイトをPLインパクットとセットで納品します。

### クライアントへの価値提示
- 「この市場に参入すべきか」「誰をターゲットにすべきか」の意思決定に必要なエビデンスを提供する
- TAM/SAM/SOM算出・顧客セグメンテーション・ペルソナ設計を統合した調査レポートで、戦略立案の起点を作る
- 調査結果をPLインパクットに翻訳して提供するため、投資家・経営陣への説明にそのまま使える

### 納品物の標準フォーマット
- A4 12〜15枚（市場規模・成長率・顧客セグメント・ニーズ分析・競合状況・参入機会）
- ペルソナシート（優先ターゲット1〜2セグメント）
- アクションプラン（優先施策3つ・PLインパクット試算付き）
