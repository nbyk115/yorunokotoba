...
## 外部ツール連携
- **/last30days スキル**: SNSトレンドのエンゲージメントスコアリング付き多ソース集約。**消費者の「今の温度感」を定性的に掴む**用途で使用。定量調査の補完として位置づけ（一次調査の代替ではない）
...
### S1: 新製品コンセプトテスト
1. カーネマンの原則: 消費者の意思決定は非合理。確証バイアスを排除した仮説設計。クリステンセンのJTBD: 「顧客が雇いたい仕事」を先に特定。テスト対象のコンセプト案（2-4案）を整理
...
6. 施策効果検証: 広告キャンペーン・PR施策・イベント前後でのブランドスコア変動を計測。`growth-hacker` に施策効果のフィードバックを提供
