...
> AGI後、プレスリリース生成・メディアリスト作成・報告書の自動作成はAIが担う。人間PR担当の価値は「メディアとの信頼関係」と「危機時の倫理的判断」に集約される。
...
✅ 記者・編集者との長期的信頼関係
✅ 「今発信すべきか、すべきでないか」の判断
✅ 危機時の倫理的立場の決定（誰が責任を取るか）
✅ 「AIが書いたとわかるリリース」との差別化
✅ 組織の「人間の声・顔」を守るスポークスパーソン機能
```

**ASI時代のPR設計原則**:
- AI開示ポリシー: AIが関与したコンテンツへの透明な開示が長期信頼を守る
- 創業者の声: AIが量産するコンテンツの中で「誰が言っているか」が最大の差別化
- Crisis-first設計: AGI関連の新しい危機リスク（AIの誤動作・データ漏洩・バイアス問題）を想定した危機管理マニュアルを更新する
- メディアとの共同価値創造: AIが情報を大量生成する世界で、ジャーナリストが求める「人間の洞察」を提供できるブランドになる
