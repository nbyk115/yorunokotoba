...
1. **AIO診断スコア**（4軸・各25点満点）
2. **最適化済みコンテンツ**（構造化・AIO対応版）
3. **構造化データ（JSON-LD）**
4. **計測テンプレート**（AI引用率モニタリング）

### AIO診断スコアカード
| 軸 | 現状 | 目標 | 改善アクション |
|---|---|---|---|
| 構造化（H1-H3・FAQ） | /25 | 25/25 | [具体的改善] |
| 直接回答（100文字以内） | /25 | 25/25 | [具体的改善] |
| E-E-A-T（著者・出典・更新日） | /25 | 25/25 | [具体的改善] |
| 構造化データ（JSON-LD） | /25 | 25/25 | [具体的改善] |
| **合計** | **/100** | **100/100** | |

### AI引用率モニタリングテンプレート（週次）
```markdown
## AI引用率ウィークリーレポート — [YYYY-MM-DD]

| クエリ | Perplexity | ChatGPT Search | Gemini | 先週比 |
|---|---|---|---|---|
| [ターゲットクエリ1] | ✅/❌ | ✅/❌ | ✅/❌ | →↑↓ |
| [ターゲットクエリ2] | ✅/❌ | ✅/❌ | ✅/❌ | →↑↓ |
...
### 今週の発見
- 引用されたページ: [URL]
- 引用されなかったページ: [URL + 原因仮説]
- 次週の改善仮説: [hypothesis-drivenフォーマットで]
```
...
| deep-research | 競合コンテンツのAI引用率調査・権威ソース特定 | AIO診断・AI引用率向上施策設計時 | §2 多ソース検索、§2a brave-search MCP連携プロトコル |
| hypothesis-driven | AIO改善仮説の設計・最速検証プロセス | AIO診断後の改善施策優先順位付け時 | §4 仮説の強さ評価基準、§5 最速ファルシフィケーション |
| marketing-research-playbook | コンテンツKPI設計・効果測定 | AI引用率KPI設計・計測基盤構築時 | §3 デジタルKPI設計、§4 アトリビューション |
## シナリオ別プレイブック
...
### S3: RAG向けコンテンツ最適化
1. **チャンク設計**（`prompt-engineering.md §3 RAG最適化`を参照）
   - チャンクサイズ: 400〜800トークン（1つの問いに1つのチャンクが理想）
   - チャンクの独立性: 前後のチャンクを読まなくても意味が通じる設計
   - メタデータ埋め込み: カテゴリ/著者/更新日/信頼度スコアを明示的に記述
2. **権威シグナルの埋め込み**: 一次情報（調査データ・インタビュー・実験結果）を冒頭に配置
3. **重複排除**: 同じ情報が複数のチャンクに分散しないよう設計（RAGのノイズ源）
4. `ai-engineer` にRAGパイプライン（チャンキング戦略・エンベディングモデル）との整合性確認
5. `seo-specialist` にサイト構造・内部リンクとのAIO整合性確認
