...
2. WebSearch で参照 URL を 3-5 件取得（例: 「UI reference [業種/用途]」「LP wireframe [業種]」）。URL とレイアウト構造を言語化して設計根拠とする
3. 人間が Lazyweb For Humans（lazyweb.com）/ `refero.design` をブラウザ閲覧している場合は、その構造情報を受け取って補完
4. DESIGN.md §12.4 業界別推奨参照（IR / セールス / 1 枚絵 / LP / ダッシュボード）で補完
5. 参照根拠を制作開始時に 1-2 行で言語化（なぜこのレイアウト / 情報密度 / 遷移か）

注: MCP 経由でコンテナに参考画像を取り込む経路は本環境で塞がれている（Lazyweb MCP 不採用 / Canva MCP は画像 403）。「画像を目視参照」ではなく「URL と構造情報を言語化して参照根拠とする」のがこの環境の標準。参照ゼロでワイヤー出すと visual ループ再発（30+ ラウンド）リスク。
...
### AI 接続点マッピング（2026-05-15 追加、btrax「AI 時代のデザイン経営」変化 2 統合）

AI 実装前提の UX デザインプロセス。全画面設計フェーズで以下 3 項目を必須チェック（出典: Brandon K. Hill btrax 2025-08-06、INFERENCE）:

- (a) AI 出力領域の明示: AI が動的生成するコンテンツ領域（レコメンド / 生成テキスト / パーソナライズ表示）を画面上で明示設計
- (b) ユーザー上書き・拒否権: AI 結果をユーザーが編集 / 上書き / 拒否できる UI を担保（メルカリ AI 商品説明文 + 編集権限が好例）
- (c) AI 不確実性の可視化: explainability UI で AI 出力の確信度 / 根拠 / 限界を提示、ブラックボックス化を回避

= 「AI が入って当たり前」の設計。YouTube/Netflix レコメンド、メルカリ AI アシスト、温泉旅館の極パーソナライズ接客が同型。関根さん N&Y Craft では「今日の一杯」レコメンド / AI 生成ビール説明文 + 編集 UI / 常連パーソナライズ画面に適用候補（技術スタック確認後）。
