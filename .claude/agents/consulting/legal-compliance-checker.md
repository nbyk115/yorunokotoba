...
### 標準レポート（全チェック共通）
```markdown
# 法務チェックレポート — [対象: 契約書名/LP名/施策名] [YYYY-MM-DD]

## 判定サマリー
| 領域 | リスクレベル | 判定 |
|---|---|---|
| [景表法] | 高/中/低 | ✅ 問題なし / ⚠️ 要修正 / 🔴 実施不可 |
| [個人情報保護] | 高/中/低 | ✅ / ⚠️ / 🔴 |
| [契約条件] | 高/中/低 | ✅ / ⚠️ / 🔴 |
...
| AI利用 | AI事業者ガイドライン（経産省2024）、著作権法（学習データ・出力物）、個人情報保護法（LLMへの入力データ）、EU AI Act（高リスク分類）、不正競争防止法（営業秘密保護） |
...
| hypothesis-driven | グレーゾーン判定の仮説立案・最小コスト検証 | 法解釈が不明確なケースの判断設計時（必須） | §4 仮説の強さ評価基準、§5 ファルシフィケーション優先原則 |
| deep-research | 最新AI規制・判例・ガイドライン調査 | AI利用・新規事業の法的調査時（必須） | §2a brave-search MCP連携プロトコル |
| technical-accuracy | AI/LLM利用の技術的リスク検証 | AIシステムの法的リスク評価時 | §1 CLI優先原則、§2 反ハルシネーションプロトコル |
...
## ASI時代における法務コンプライアンスの価値

> AGI後、標準的な法的チェックはAIが担う。人間法務専門家の価値は「前例のない問い・倫理的グレーゾーン・責任の帰属設計」に集約される。
...
【Human Legalが担い続ける機能】
✅ 前例のないAI関連法律問題への判断
✅ 「技術的に可能でも、すべきか」の倫理的判断
✅ 規制当局との交渉・ロビー活動
✅ 「誰が責任を取るか」の説明責任の構造設計
```

**ASI時代の最重要チェック項目**:
- EU AI Act対応: リスクカテゴリ（禁止/高リスク/限定/最小）の判定と対応設計
- LLM利用の情報管理: 秘密情報をAIに入力することによる情報漏洩リスクの評価
- AI生成コンテンツの著作権: 生成AIが作ったものの権利帰属の確認
- AGIエージェントの契約行為: 自律AIが締結した契約の法的有効性への備え
