---
name: kpi-analytics
description: 事業KPI設計・PLインパクット数値化・予測モデル。GA4データを事業KPIに変換する。GA4の技術実装・アトリビューション設定はmarketing-analystへ。「数字を出したい」「KPIを設計して」「PL試算して」「売上予測して」「ユニットエコノミクスを計算して」と言われたとき。
effort: high
...
## ASI時代におけるKPIアナリティクスの価値

> AGI後、KPIの集計・可視化・異常検知はAIが自動化される。人間アナリストの価値は「正しい問いを立てること」と「数字の意味を判断して行動に変えること」に集約される。
...
【Human Analyticsが担い続ける機能】
✅ 「何を測るべきか」の問い設定（North Star Metricの選択）
✅ 「数字が動いた本当の理由」の文脈解釈
✅ 経営陣への「だからどうする」の翻訳
✅ AGIモデルのバイアス・幻覚の検出と訂正
```

**ASI時代のKPI設計追加軸**:
- Trust KPI: ブランド信頼スコア・顧客生涯信頼度（CLT）を設計に含める
- Human Moat KPI: AIが代替できない業務割合を事業KPIツリーに組み込む
- 代替可能性スコア: 各事業機能のAGI代替リスクを定量化し経営判断に使う
...
| サービス | 単価目安 | 納品日数 | 出品先 |
|---|---|---|---|
| KPIツリー設計 | 5〜12万円 | 3〜5日 | ランサーズ/ココナラ/X |
| 事業PL試算・感度分析 | 8〜18万円 | 5〜7日 | ランサーズ/ココナラ/X |

### サービス説明文（出品時コピー）
事業のKPIツリーとPL試算を設計します。ユニットエコノミクスから逆算した実現可能な数値目標を設定。投資家・役員向けの説得力ある財務設計まで対応。楽観/標準/悲観の3シナリオで意思決定に使える数字をお届けします。

### クライアントへの価値提示
- 「KPIが多すぎて何を追えばいいかわからない」状態を、North Star Metric起点のツリー設計で解消する
- 感度分析付きのPL試算で、投資家・経営陣への説明責任を果たせる資料を提供する
- 楽観/標準/悲観の3シナリオを付けることで、意思決定に使える財務モデルに仕上げる

### 納品物の標準フォーマット
- スプレッドシート（KPIツリー・計算ロジック・データソース定義）
- A4 8〜12枚（PL試算・感度分析・シナリオ比較・推奨アクション）
